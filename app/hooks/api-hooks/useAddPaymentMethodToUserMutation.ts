...
function useAddPaymentMethodToUserMutation(): AddPaymentMethodToUserMutationInterface {
  const queryClient = useQueryClient();
  const queryKey = ['user'];
  const { mutate, isPending, isSuccess } = useMutation({
    mutationFn: (data: PaymentMethodInterface) => UserApi.addPaymentMethod(data),
    onSuccess: () => {
      enqueueSnackbar(`Payment method successfully added`, {
        variant: 'success',
...
export default useAddPaymentMethodToUserMutation;
